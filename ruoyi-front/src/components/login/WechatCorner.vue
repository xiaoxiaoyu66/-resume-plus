<template>
  <div class="wechat-corner" @click="$emit('open')" @mouseenter="hover = true" @mouseleave="hover = false">
    <div class="corner-bg" :class="{ hover }">
      ...
      <div class="corner-tooltip" :class="{ show: hover }">微信扫码登录</div>
    </div>
  </div>
</template>
...
.wechat-corner {
  ...
  .corner-bg {
    ...
    &:hover .qr-icon { transform: scale(1.1); }
...
@media (max-width: 480px) {
  ...
  .corner-bg .qr-icon { right: 10px; bottom: 10px; width: 20px; height: 20px; }
}
</style>
